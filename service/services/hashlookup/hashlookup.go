...
func (h *Hashlookup) Processing(partial bool) (int, interface{}, map[string]string, map[string]interface{},
	...
	if h.maxFileSize != 0 && h.maxFileSize < file.Len() {
		status, file, httpMsgAfter := h.generalFunc.IfMaxFileSizeExc(h.returnOrigIfMaxSizeExc, h.serviceName, h.methodName, file, h.maxFileSize)
		fileAfterPrep, httpMsgAfter := h.generalFunc.IfStatusIs204WithFile(h.methodName, status, file, isGzip, reqContentType, httpMsgAfter, true)
	...
	if isMal {
		...
		if h.methodName == utils.ICAPModeResp {
			...
			logging.Logger.Info(utils.PrepareLogMsg(h.xICAPMetadata, h.serviceName+" service has stopped processing"))
			return utils.OkStatusCodeStr, h.httpMsg.Response, serviceHeaders, msgHeadersBeforeProcessing, msgHeadersAfterProcessing, vendorMsgs
		} else {
	...
	logging.Logger.Info(utils.PrepareLogMsg(h.xICAPMetadata, h.serviceName+" service has stopped processing"))
	return utils.OkStatusCodeStr, h.generalFunc.ReturningHttpMessageWithFile(h.methodName, scannedFile),
		serviceHeaders, msgHeadersBeforeProcessing, msgHeadersAfterProcessing, vendorMsgs
}
