...
func (c *Clamav) Processing(partial bool, IcapHeader textproto.MIMEHeader) (int, interface{}, map[string]string, map[string]interface{},
	...
	logging.Logger.Debug(utils.PrepareLogMsg(c.xICAPMetadata, c.serviceName+" service has started processing"))
	...
	if c.methodName == utils.ICAPModeResp {
		if c.httpMsg.Response != nil {
			if c.httpMsg.Response.StatusCode == 206 {
				logging.Logger.Debug(utils.PrepareLogMsg(c.xICAPMetadata, c.serviceName+" service has stopped processing byte range received"))
	...
	if err != nil {
		...
		logging.Logger.Debug(utils.PrepareLogMsg(c.xICAPMetadata, c.serviceName+" service has stopped processing"))
	...
	logging.Logger.Debug(utils.PrepareLogMsg(c.xICAPMetadata, c.serviceName+" file name : "+fileName))
	...
	c.FileHash = fileHash
	logging.Logger.Debug(utils.PrepareLogMsg(c.xICAPMetadata, c.serviceName+" file hash : "+fileHash))
