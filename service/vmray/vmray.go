...
func NewVmrayService(serviceName string, logger *logger.ZLogger) *Vmray {
	return &Vmray{
...
func (v *Vmray) SubmitFile(f *bytes.Buffer, filename string) (*dtos.SubmitResponse, error) {
	...
	if err != nil {
		zLog.Logger.Error().Msgf("service: vmray: failed to do request: %s", err.Error())
		return nil, err
	}
	...
	return toSubmitResponse(&sresp), nil
}
...
func (v *Vmray) GetSampleFileInfo(sampleID string, filemetas ...dtos.FileMetaInfo) (*dtos.SampleInfo, error) {
	...
	return toSampleInfo(&sampleResp), nil
}
...
func (v *Vmray) GetSubmissionStatus(submissionID string) (*dtos.SubmissionStatusResponse, error) {
	...
	return toSubmissionStatusResponse(&ssResp), nil
}
...
func (v *Vmray) SendFileApi(f *bytes.Buffer, filename string, reqURL string) (*http.Response, int, bool, string, error) {
