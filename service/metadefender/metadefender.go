...
func NewMetaDefenderService(serviceName string, logger *logger.ZLogger) *MetaDefender {
	return &MetaDefender{
...
func (m *MetaDefender) SubmitFile(f *bytes.Buffer, filename string) (*dtos.SubmitResponse, error) {
	...
	if err != nil {
		zLog.Logger.Error().Msgf("service: metadefender: failed to do request: %s", err.Error())
		return nil, err
	}
...
func (m *MetaDefender) GetSampleFileInfo(sampleID string, filemetas ...dtos.FileMetaInfo) (*dtos.SampleInfo, error) {
	...
	return toSampleInfo(&sampleResp, fm, m.FailThreshold), nil
...
func (m *MetaDefender) SendFileApi(f *bytes.Buffer, filename string, reqURL string) (*http.Response, int, bool, string, error) {
